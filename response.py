...
class Response:
    ...
    def response(self):  # Computer move
        ...
        o_move = Response.potential_win(self, "O")  # Contains winning computer move
        ...
        if move_X_no >= 2 and type(x_move) == int:  # If there is a risky line blok it
        ...
        elif move_X_no == 1 and self.state.index("X") in (1, 3, 5, 7):  # if "X" in a center of the outer line...
            ...
            two = Response.two_lines(self, "O", 0)
            print(7777, two)
            if not two:
                self.state[self.state.index(" ")] = "O"
                print(8)
    ...
    def two_lines(self, who, level):  # Find empty field where adding "O" would create 2 lines of 2x "O" and
        ...
        print("recursion")
        return Response.two_lines(self, "X", level)  # Recursive check for "X"
